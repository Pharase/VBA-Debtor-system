...
def process_excel_file(file_path):
    try:
        ...
        for i in range(7, 7 + cell_count):
            # Construct the range representing the entire row from column A to EJ
            row_range = ws.range(f'A{i}:EJ{i}').value
...
main_data = r"C:\Pam_card\_Data_CutOff_v18.xlsx"
...
summary_temp = r"C:\Pam_card\summary\_summary_template_v3.xlsx"
destb = xw.Book(summary_temp)
dests = destb.sheets['transaction_record']

dests.range('EG17').value = main_data
...
# Construct the range representing the entire row from column A to CV
    row_range = ws.range(f'A{i}:CY{i}').value
...
# Construct the range representing the entire row from column CY to DW
    row_range = ws.range(f'DB{i}:DZ{i}').value
    if len(str(row_range[0])) == 8:
        temp_value = str(int(row_range[0]))
        row_range[0] = '0' + temp_value
    cell_values.append(row_range)
...
# Paste the data into the target sheet
dests.range(f'DB2:DB{count_change}').number_format = '@'
dests.range(f'DB2').value = cell_values

output_file = os.path.join(r"C:\Pam_card\summary", f"summary_data_file_{formatted_date}.xlsx")

# Update Cost
dests = destb.sheets['Cost']
Cost_last_row = dests.range('F2').value

for i in range(2,Cost_last_row):
    dests.range(f'F{i}').formula2 = (
        '=IFERROR('
            'INDEX(SORT(FILTER(transaction_record!E:P, transaction_record!A:A = [@[pam_code]]), 12, -1), 1, 1), '
            'XLOOKUP([@[pam_code]], '
                'INDIRECT("\'" & transaction_record!$ED$17 & "\'!" & transaction_record!$ED$20 & "[pam_code]"), '
                'INDIRECT("\'" & transaction_record!$ED$17 & "\'!" & transaction_record!$ED$20 & "[transferred_status]")'
            ')'
        ')'
    )
    dests.range(f'D{i}').formula2 = f"=IFERROR(MIN(FILTER(transaction_record!DP:DP,transaction_record!A:A=[@[pam_code]])),[@[bid_price_excluding_vat]])"

dests.book.app.calculate()

# Replace formulas with values
for i in range(2, Cost_last_row):
    dests.range(f'F{i}').value = dests.range(f'F{i}').value
    dests.range(f'D{i}').value = dests.range(f'D{i}').value
